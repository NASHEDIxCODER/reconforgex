"""
Structured logging for the ReconForgeX framework.

Provides a ``ReconForgeXLogger`` singleton that writes timestamped, level-aware
...
def get_logger(
    ...
    """Return the global ``ReconForgeXLogger`` singleton.
    ...
    global _logger
    if _logger is None:
        _logger = ReconForgeXLogger(name=name, level=level, log_file=log_file)
    return _logger
